...
- **One DMG per architecture**, 43 MB and 45 MB. A universal bundle is the *sum* of both, so
  shipping them apart halves what anyone downloads while costing the host and the repository
  the same — both files exist either way. `build-mac.sh X.Y.Z universal` still produces the
  combined 85 MB bundle if one download that runs everywhere is worth more than the size.
- **`RuntimeIdentifiers` must be declared in the csproj**, not passed on the command line,
  where MSBuild reads the list as one malformed RID and fails with NETSDK1083. `publish` then
  runs **without** `-r`: passing one builds a single architecture and silently produces
  exactly the bundle this arrangement exists to avoid.
- **The script checks what it built.** A bundle with the wrong slices looks identical from the
  outside and only fails on the machines that cannot run it — which are, by definition, not
